...
UN178Driver sternBridge = UN178Driver(40, 42, 44, 41, 43, 45); // Stern motor and rudder respectively - Digital/Digital/PWM order of pins // Output pins for motor drivers
...
constexpr int16_t rudderADCMinThreshold = 293;  // Rudder -37 degrees (Starboard) 494
constexpr int16_t rudderADCMiddleThreshold = 516;
constexpr int16_t rudderADCMaxThreshold = 660;  // Rudder 57 degrees (Port) 554 = /
...
//EXPERIMENT METER TIME BETWEEN ARDUINO MAVROS WAY
int decimalValueFromRaspberryGPIO = 0;
//EXPERIMENT METER TIME BETWEEN ARDUINO MAVROS WAY

void setup() {
  //Serial3.begin(115200);  // MAvlink
  Serial3.begin(57600);  // MAvlink
  ...
  pinMode(experimentPin3, INPUT);
  pinMode(experimentpin4, INPUT);
  pinMode(experimentpin5, INPUT);
  pinMode(experimentpin6, INPUT);
  pinMode(experimentpin7, INPUT);
  ...
  //linha abaixo é código para teste de envio de parâmetro
  randomSeed(analogRead(0)); // Semente para geração de números randômicos
}
...
void loop() {
  ...
  GetSerialInput();
  //send_heartbeat(); // Envie um "heartbeat" antes de enviar o parâmetro
  //Serial.print("antes send_mavlink ");
    //send_mavlink(posicao_vela, 37.587f);


    //EXPERIMENT METER TIME BETWEEN ARDUINO MAVROS WAY

    int state3 = digitalRead(experimentPin3);
    int state4 = digitalRead(experimentpin4);
    int state5 = digitalRead(experimentpin5);
    int state6 = digitalRead(experimentpin6);
    int state7 = digitalRead(experimentpin7);
    
    
    int newDecimalValueFromRaspberryGPIO = -1;

    newDecimalValueFromRaspberryGPIO = ConvertBinaryToInt(state3, state4, state5, state6, state7);

    //EXPERIMENT METER TIME BETWEEN ARDUINO MAVROS WAY



    float randomNum = random(101);  
    //send_mavlink(corrente_leme, randomNum);
    randomNum = random(101);
    //send_mavlink(motor_leme, randomNum);
    randomNum = random(101);
    //send_mavlink(corrente_vela, randomNum);
    randomNum = random(101);
    //send_mavlink(motor_vela, randomNum);
    randomNum = random(101);
    //send_mavlink(posicao_leme, randomNum);
    if(newDecimalValueFromRaspberryGPIO != decimalValueFromRaspberryGPIO)
    {
      send_mavlink(posicao_leme, decimalValueFromRaspberryGPIO);
      decimalValueFromRaspberryGPIO = newDecimalValueFromRaspberryGPIO;
    }
    randomNum = random(101);
    //send_mavlink(posicao_vela, randomNum);
    // send_mavlink(corrente_leme, 2006.22f);
    // send_mavlink(motor_leme, -2845.3f);////
    // send_mavlink(corrente_vela, -5.0f);
    // send_mavlink(motor_vela, 1.1f);
    // send_mavlink(posicao_leme, 45.5f);
    // send_mavlink(posicao_vela, 8500.5f);
    
}
    
    
//EXPERIMENT METER TIME BETWEEN ARDUINO MAVROS WAY
int ConvertBinaryToInt(int char1, int char2, int char3, int char4, int char5){
  int decimalValue = 0;

  char binaryString[6]; // 3 bits + 1 para o caractere nulo
  binaryString[0] = (char1 == 1) ? '1' : '0';
  binaryString[1] = (char2 == 1) ? '1' : '0';
  binaryString[2] = (char3 == 1) ? '1' : '0';
  binaryString[3] = (char4 == 1) ? '1' : '0';
  binaryString[4] = (char5 == 1) ? '1' : '0';
  binaryString[5] = '\0'; // Adiciona o caractere nulo ao final da string
...
void RudderControl(int rudder_angle_desired) {  
  ...
  //@//Serial.print("\nRudder ADC: "); Serial.print(rudder_adc_reading);
  //@//Serial.print("\tAngle: "); Serial.println(rudder_angle_present);
  ...
  if (rudder_error > -8 && rudder_error < 8) rudder_output_pwm = 0; //Prevents excessive microadjustments from the actuator
...
void send_heartbeat() {
    // Criar uma mensagem MAVLink do tipo HEARTBEAT
    mavlink_message_t msg;
    mavlink_heartbeat_t heartbeat;

    heartbeat.type = MAV_TYPE_GCS; // Tipo de sistema (pode ser ajustado conforme necessário)
    heartbeat.autopilot = MAV_AUTOPILOT_INVALID; // Autopiloto (ajuste conforme necessário)
    heartbeat.base_mode = 0; // Modo base
    heartbeat.custom_mode = 0; // Modo customizado
    heartbeat.system_status = MAV_STATE_ACTIVE; // Status do sistema (ajuste conforme necessário)

    // Empacotar a mensagem MAVLink
    mavlink_msg_heartbeat_encode(255, 0, &msg, &heartbeat);

    // Converter a mensagem para um buffer para envio
    uint8_t buf[MAVLINK_MAX_PACKET_LEN];
    uint16_t len = mavlink_msg_to_send_buffer(buf, &msg);

    // Enviar a mensagem HEARTBEAT via Serial
    for (int i = 0; i < len; i++) {
        Serial3.write(buf[i]);
    }
}
...
void send_mavlink(int param, float data){
  ...
  //Serial.print("send_mavlink param ");Serial.print(param);Serial.print(" data ");Serial.print(data);
  ...
  switch(param){
    case corrente_leme:
    {
      mavlink_msg_param_set_pack(2, 1, &msg, 1, 1, "RUDDER_CURRENT", data, MAV_VAR_FLOAT);
    }
    break;

    case corrente_vela:
    {
      mavlink_msg_param_set_pack(2, 1, &msg, 1, 1, "SAIL_CURRENT", data, MAV_VAR_FLOAT);
    }
    break;

    case posicao_leme:
    {
      mavlink_msg_param_set_pack(2, 1, &msg, 1, 1, "RUDDER_ANGLE", data, MAV_VAR_FLOAT);
    ...
    case posicao_vela:
    {
      mavlink_msg_param_set_pack(2, 1, &msg, 1, 1, "SAIL_ANGLE", data, MAV_VAR_FLOAT);
    }
    break;

    case motor_leme:
    {
      mavlink_msg_param_set_pack(2, 1, &msg, 1, 1, "RUDDER_MOTOR", data, MAV_VAR_FLOAT);
    }
    break;

    case motor_vela:
    {
      mavlink_msg_param_set_pack(2, 1, &msg, 1, 1, "SAIL_MOTOR", data, MAV_VAR_FLOAT);
    }
    break;
  }  
...
void MAVLinkToPixhawk(MAVLink_options option, float data) {
  ...
  switch (option) { 
    case MAVLink_options::rudder_angle:
    {
      ...
      Serial.print("RUDDER_ANGLE: "); Serial.println(data);
    }
    break;
    ...
    case MAVLink_options::sail_angle:
    {
      ...
      Serial.print("SAIL_ANGLE: "); Serial.println(data);
    }
    break;
    ...
    case MAVLink_options::rudder_pwm:
    {
      ...
      Serial.print("RUDDER_PWM: "); Serial.println(data);
    }
    break;
    ...
    case MAVLink_options::sail_pwm:
    {
      ...
      Serial.print("SAIL_PWM: "); Serial.println(data);
    }
    break;
  }  
